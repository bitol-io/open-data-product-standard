...
* Add optional `tags`, `customProperties` and `authoritativeDefinitions` on `outputPorts[].sbom[]` entries, so each SBOM can be labelled, carry vendor-specific context, and link to its authoritative build record. The object stays closed (`additionalProperties: false`); only its property list grows, and every valid v1.1.0 document stays valid — [RFC-0061](https://github.com/bitol-io/tsc/blob/main/rfcs/approved/odps-v1.1.0/0061-sbom-metadata.md).
